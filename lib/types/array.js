...
Monadidator.is.array = (tmpl) => {
  ...
  return entries.reduce(
    (validator, [key, inner]) => {
      return validator.chain((xs) =>
        ArrayValidator.where(key, inner).chain((value) => {
          return new ArrayValidator((state, ok) => {
            const _xs = [...xs];
            _xs[key] = value;
            return ok(_xs, {...state, map: true});
          });
        })
      );
    },
    isArray().chain(() => new ArrayValidator((state, ok) => ok([], state)))
  );
};
